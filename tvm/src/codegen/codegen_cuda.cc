...
void CodeGenCUDA::VisitExpr_(const Broadcast* op, std::ostream& os) {   // NOLINT(*)
  ...
  if (op->type.is_float() && op->type.bits() == 16 && op->lanes == 8) {
    // make_int4
    os << "make_int4(0,0,0,0)";
    return;
  }

  if (op->type.is_float() && op->type.bits() == 16 && op->lanes == 4) {
    // make_int2
    os << "make_int2(0,0)";
    return;
  }

  if (op->type.is_float() && op->type.bits() == 16 && op->lanes == 2) {
    // make_int
    os << "(int)(0x0000)";
    return;
  }
