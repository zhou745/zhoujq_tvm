...
std::string CodeGenC::Finish() {
  return decl_stream.str() + user_defined_head + stream.str();
}
...
void CodeGenC::VisitExpr_(const Call *op, std::ostream& os) {  // NOLINT(*)
  if (op->call_type == Call::Extern ||
    ...
    os << ")";
  } else if(op->is_intrinsic(Call::wmma_fragment)){
      os<<"wmma::fragment<wmma::";
      os<<op->args[0].as<StringImm>()->value<<", 16, 16, 16, "<<op->args[1].as<StringImm>()->value;
      if(op->args[0].as<StringImm>()->value=="matrix_a"){
         os<<", wmma::"<<op->args[2].as<StringImm>()->value<<"> "<<op->args[3].as<StringImm>()->value<<"[";
         PrintExpr(op->args[4],os);
         os<<"]";
      } else if(op->args[0].as<StringImm>()->value=="matrix_b") {
         os<<", wmma::"<<op->args[2].as<StringImm>()->value<<"> "<<op->args[3].as<StringImm>()->value<<"[";
         PrintExpr(op->args[4],os);
         os<<"]";
      } else if(op->args[0].as<StringImm>()->value=="accumulator"){
         os<<"> "<<op->args[2].as<StringImm>()->value<<"[";
         PrintExpr(op->args[3],os);
         os<<"][";
         PrintExpr(op->args[4],os);
         os<<"]";
      }
  } else if(op->is_intrinsic(Call::wmma_load_matrix_sync)){
         os<<"wmma::load_matrix_sync("<<op->args[0].as<StringImm>()->value<<"[";
         ...
         PrintExpr(op->args[4],os);
         os<<")";
  } else if(op->is_intrinsic(Call::wmma_mma_sync)){
         os<<"wmma::mma_sync("<<op->args[0].as<StringImm>()->value<<"[";
         PrintExpr(op->args[4],os);
         os<<"][";
         PrintExpr(op->args[5],os);
         os<<"],"<<op->args[1].as<StringImm>()->value<<"[";
         PrintExpr(op->args[4],os);
         os<<"],"<<op->args[2].as<StringImm>()->value<<"[";
         PrintExpr(op->args[5],os);
         os<<"],"<<op->args[3].as<StringImm>()->value<<"[";
         PrintExpr(op->args[4],os);
         os<<"][";
         PrintExpr(op->args[5],os);
         os<<"])";
  } else if(op->is_intrinsic(Call::wmma_store_matrix_sync)){
         os<<"wmma::store_matrix_sync(";
  ...
  } else if(op->is_intrinsic(Call::wmma_fill_fragment)){
         os<<"wmma::fill_fragment("<<op->args[0].as<StringImm>()->value<<"[";
         PrintExpr(op->args[1],os);
         os<<"][";
         PrintExpr(op->args[2],os);
         os<<"],"<<"static_cast<"<<op->args[3].as<StringImm>()->value<<">(";
         PrintExpr(op->args[4],os);
         os<<"))";
  } else if(op->is_intrinsic(Call::int4_copy)){
         os<<"*(int4*)(";
         PrintExpr(op->args[0],os);
         os<<"+";
         PrintExpr(op->args[1],os);
         ...
         PrintExpr(op->args[3],os);
         os<<")";
  } else if(op->is_intrinsic(Call::include_cpp_head)){
        if(user_defined_head==""){
          user_defined_head+="#include\"";
          user_defined_head+=op->args[0].as<StringImm>()->value;
          user_defined_head+="\"\n";}
  } else if(op->is_intrinsic(Call::relu)) {
        os<<"for(int index=0;index<8;index++){\n(";
        PrintExpr(op->args[0],os);
        os<<"+"<<op->args[1]<<")";
        os<<"[index]=(";
        PrintExpr(op->args[0],os);
        os<<"+"<<op->args[1]<<")";
        os<<"[index]>static_cast<half>(0.)? (";
        PrintExpr(op->args[0],os);
        os<<"+"<<op->args[1]<<")";
        os<<"[index]:static_cast<half>(0.);\n}";
  } else if(op->is_intrinsic(Call::wmma_copy_int4)) {
        os<<"wmma_load_int4("<<op->args[0].as<StringImm>()->value<<"[";
        PrintExpr(op->args[1],os);
        os<<"].x,";
        PrintExpr(op->args[2],os);
        os<<",";
...
void CodeGenC::VisitStmt_(const Evaluate *op) {
  ...
  if (vid != "") {
    this->PrintIndent();
    if((std::string)(call->name)=="include_cpp_head"){
        this->stream << vid << "\n";
    } else {
        this->stream << vid << ";\n";
    }
