...
from conv2d_fp16 import conv2d_NHWC_fp16,schedule_NHWC_fp16

@autotvm.register_topi_compute(nn.conv2d, ['cuda', 'gpu'], ['direct', 'winograd', 'int8','fp16'])
def conv2d_cuda(cfg, data, kernel, strides, padding, dilation, layout='NCHW', out_dtype='float32'):
    ...
    if cfg.template_key =='fp16':
        return conv2d_NHWC_fp16(cfg,data,kernel,strides,padding,dilation,layout,out_dtype)
...
def schedule_conv2d_nhwc_cuda(cfg, outs):
    ...
    def _callback(op):
        if op.tag == 'conv2d_nhwc':
            schedule_direct_cuda(cfg, s, op.output(0))
        if op.tag == "conv2d_NHWC_HMMA" or op.tag == "conv2d_NHWC_HFMA" or op.tag == "conv2d_Tensor_core_copy":
            schedule_NHWC_fp16(cfg,s,op.output(0))
    traverse_inline(s, outs[0].op, _callback)
    print(s.outputs)
    return s
