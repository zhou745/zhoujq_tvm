...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck=True,last_filter=16):
    ...
    """
    if bottle_neck:
        bn1 = sym.batch_norm(data=data, epsilon=2e-5, name=name + '_bn1',axis=3)
        ...
        bn2 = sym.batch_norm(data=conv1, epsilon=2e-5, name=name + '_bn2',axis=3)
        ...
        bn3 = sym.batch_norm(data=conv2, epsilon=2e-5, name=name + '_bn3',axis=3)
...
def resnet(units, num_stages, filter_list, num_classes, image_shape,
    ...
    if height <= 32:            # such as cifar10
        ...
        body = sym.max_pool2d(data=body, pool_size=(3, 3), strides=(2, 2), padding=(1, 1),layout="NHWC")
    #body = residual_unit2(
    ...
    for i in range(num_stages):
        body = residual_unit(
            ...
            False, name='stage%d_unit%d' % (i + 1, 1), bottle_neck=bottle_neck,last_filter=filter_list[i])
        for j in range(units[i]-1):
            body = residual_unit(
                body, filter_list[i+1], (1, 1), True,
                name='stage%d_unit%d' % (i + 1, j + 2), bottle_neck=bottle_neck,last_filter=filter_list[i+1])
