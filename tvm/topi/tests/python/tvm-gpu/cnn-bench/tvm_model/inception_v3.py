...
def Conv(data, num_filter, kernel=(1, 1), stride=(1, 1), pad=(0, 0), name=None, suffix=''):
    ...
    bn = int8_wrapper(sym.batch_norm, data=conv, name='%s%s_bn' % (name, suffix), epsilon=2e-5)
...
def Pooling(data, kernel, stride, pad, pool_type, name):
    if pool_type == 'max':
        ...
        return int8_wrapper(sym.avg_pool2d, data=data, pool_size=kernel, strides=stride, padding=pad, name=name,
