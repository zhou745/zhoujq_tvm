...
def get_feature(internel_layer, layers, filters, batch_norm = False, **kwargs):
    for i, num in enumerate(layers):
        for j in range(num):
            internel_layer = mx.sym.Convolution(data = internel_layer, kernel=(3, 3), pad=(1, 1), num_filter=filters[i], name="conv%s_%s" %(i + 1, j + 1),no_bias=True)
            if batch_norm:
...
def get_symbol(num_classes=0, num_layers=11, batch_norm=False, dtype='float32', **kwargs):
    """
    Parameters
    ----------
...
vgg_net = get_symbol(num_layers=16,batch_norm=False)
module_vgg = mx.mod.Module(symbol=vgg_net,context=mx.cpu(),data_names=["data"],label_names=None)