...
@autotvm.template
def conv2d(N, H, W, CI, CO, KH, KW, strides, padding, groups, scaling_factor):
    cfg = autotvm.get_config()

    CI_per_group = CI // groups
    data = tvm.placeholder((N, CI // BI, H, W, BI), name='data', dtype='int8')
    kernel = tvm.placeholder(
        (CO // BO, CI_per_group // BI, KH, KW, BO, BI), name='kernel', dtype='int8')
    ...
    ic_chunk = tvm.reduce_axis((0, CI_per_group//BI), name='ic_chunk')
    ic_block = tvm.reduce_axis((0, BI), name='ic_block')
    kh = tvm.reduce_axis((0, KH), name='kh')
    kw = tvm.reduce_axis((0, KW), name='kw')

    conv = tvm.compute(oshape, lambda n, oc_chunk, oh, ow, oc_block:
                       tvm.sum(pad_data[n, oc_chunk//groups*CI_per_group+ic_chunk, oh*stride_h+kh, ow*stride_w+kw, ic_block]
                               .astype('int32') *
    ...
    cfg.add_flop(N*CO*NH*NW*(CI*KH*KW*2)//groups)
    return s, [raw_data, kernel, output]


logging.basicConfig(level=logging.DEBUG, stream=sys.stdout)

N, H, W, CO, CI, KH, KW, strides, padding, groups, scaling_factor = 1, 17, 17, 192, 192, 7, 7, 1, 3, 4, 1.0
task = autotvm.task.create(conv2d,
                           args=(N, H, W, CO, CI, KH, KW, strides, padding, groups, scaling_factor),
                           target='cuda')
