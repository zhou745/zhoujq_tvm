...
def conv2d(N, H, W, CI, CO, KH, KW, strides, padding, scaling_factor):
    ...
    dilate_args = (1, 1, dilation, dilation, 1, 1)
    ...
    def dilate_kernel(*indices):   # This function is the same as topi.nn.dilate, but inlined
        ...
        if not_zero:
            ...
            return tvm.select(not_zero, kernel(*index_tuple), tvm.const(0.0, data.dtype))
    ...
    conv = tvm.compute(oshape, lambda n, oc_chunk, oh, ow, oc_block:
                       tvm.sum(pad_data[n, ic_chunk, oh*stride_h+kh, ow*stride_w+kw, ic_block]
                               .astype('int32') *
                               dilate_kernel(oc_chunk, ic_chunk,
    ...
    s[AA].compute_at(s[conv], rci)
    s[WW].compute_at(s[conv], rci)
...
N, H, W, CO, CI, KH, KW, strides, padding, scaling_factor = 32, 14, 14, 256, 256, 3, 3, (1, 1),(1,1),1.0
